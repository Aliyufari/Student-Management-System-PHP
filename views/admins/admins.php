<?php require('./views/includes/header.php') ?>
<?php require('./views/includes/nav.php') ?>       
<!-- Mashead header-->
<header class="masthead">
    <?php if (isset($_SESSION['user_id']) && isset($_SESSION['user_email'])): ?>
        <div class="container px-5">
            <div class="row gx-5 align-items-center">

              <div class="row mb-3 d-flex justify-content-end">
                <div class="col-3">
                  <a href=".?action=create-admin" class="btn btn-primary">Create Admin</a>
                </div>
              </div>

              <div class="row">

                <table class="table align-middle mb-0 bg-white">
                  <thead class="bg-dark" style="color: #fff; font-weight: normal;">
                    <tr>
                      <th>S/N</th>
                      <th>Name</th>
                      <th>Username</th>
                      <th>Email</th>
                      <th>Date of birth</th>
                      <th>Gender</th>
                      <th>Phone Number</th>
                      <th>Image</th>
                      <th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php if ($data['admins']):?>
                      <?php foreach ($data['admins'] as $admin):?>

                        <tr>
                          <td><?= $data['count']++; ?></td>
                          <td><?= $admin['name']; ?></td>
                          <td><?= $admin['username']; ?></td>
                          <td><?= $admin['email']; ?></td>
                          <td><?= $admin['dob']; ?></td>
                          <td><?= $admin['gender']; ?></td>
                          <td><?= $admin['phone']; ?></td>
                          <td>
                            <div class="d-flex align-items-center">
                              <img
                                  src="./public/images/profiles/admins/<?= $admin['image']; ?>"
                                  alt=""
                                  style="width: 45px; height: 45px"
                                  class="rounded-circle"
                                  />
                            </div>
                          </td>
                          <td>
                            <div class="d-flex">
                              <a 
                                href=".?action=edit-admin&admin_id=<?= $admin['id'] ?>" 
                                class="btn btn-success btn-sm btn-rounded mr-2"
                                style="margin-right: 5px;">Edit</a>

                              <form action=".?action=delete_admin" method="POST">
                                <input type="hidden" name="admin_id" value="<?= $admin['id']; ?>">
                                <button type="submit" class="btn btn-danger btn-sm btn-rounded">
                                  Delete
                                </button>
                            </form>
                            </div>
                            
                          </td>
                        </tr>
                      <?php endforeach; ?>
                    <?php endif; ?>
                  </tbody>
                </table>

              </div>
            
            </div>
        </div>
    <?php else: ?>
        
    <?php endif; ?>
</header>
<?php require('./views/includes/footer.php') ?>       
